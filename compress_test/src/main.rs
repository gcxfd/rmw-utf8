...
fn main() -> Result<(), Box<dyn std::error::Error>> {
  let _ = encode(&[]); // 先调用下，加载下字典避免影响评测速度；实际使用时候并不需要这样
...
{:>w$} : {:>w2$}"#,
      txt,
      "orginal text bytes length",
      txt_len,
      w = 33,
      w2 = 3
    );

    macro_rules! compress_ratio {
      ($tip:expr, $func:expr) => {
        ...
        let len = bytes.len();
        println!(
          "{:>wtip$} compressed length :{:>w$} , compress ratio {:>wr$}% , cost {:.2}ms",
          $tip,
          len,
          format!("{:.2}", 100.0 * (len as f64) / (txt_len as f64)),
          cost as f64 / 1000000.0,
          wtip = 15,
          wr = 6,
          w = 4,
        )
    ...
    compress_ratio!("gz(level 6)", {
      encoder.write_all(txt.as_bytes())?;
      encoder.finish()?
    });

    compress_ratio!("lzma(level 6)", { lzma::compress(txt.as_bytes(), 6)? });

    compress_ratio!("zstd(level 6)", { encode_all(Cursor::new(txt), 6)? });

    compress_ratio!("zstd(level 19)", { encode_all(Cursor::new(txt), 19)? });

    compress_ratio!("rmw-utf8", { encode(&txt.as_bytes()) });
    println!("");
  }
  Ok(())
}
